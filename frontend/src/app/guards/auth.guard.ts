...
import { RouterService } from '@bloomlocal/ng-shared';
...
export class AuthGuard implements CanActivate {
  ...
  canActivate(
    ...
    return this.store.pipe(
      ...
      map(({ signInUserSession, currentUser, changePasswordRequired }) => {
        ...
        if (isLoginPage) {
          ...
          if (authenticated) {
            this.routerService.navigateToLandingPage(currentUser.role);
            return false;
          }
        ...
        if (requiresPasswordChange) {
          ...
          if (!isPasswordResetPage) {
            this.router.navigate(['auth', 'new-password-required']);
            return false;
          }
          else {
            return true;
