...
import {
  ...
  USER_UNARCHIVE_FAIL_MESSAGE,
  USER_UNARCHIVE_SUCCESS_MESSAGE,
...
import { AlertComponent, ButtonTypes } from '../../shared/dialogs/alert/alert.component';
import { FiltersDialogComponent, FiltersDialogData } from '../../shared/dialogs/filters-dialog/filters-dialog.component';
...
export class UsersListEffects {
  ...
  onLoadUsers$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatMap((action) =>
        of(action).pipe(
          withLatestFrom(this.store.select(UsersListReducer.selectState))
        )
      ),
      switchMap(([action, listState]) => {
        ...
        return this.usersService.getUsers(param).pipe(
          map((result) => UsersListActions.onLoadUsersSuccess({ result })),
          catchError((error) =>
      ...
      return this.actions$.pipe(
        ...
        tap(([action, state]) => {
          ...
          dialogRef.afterClosed().subscribe((result) => {
            if (result) {
              this.store.dispatch(
                UsersListActions.onApplyFilters({ filters: result })
              );
            }
          });
        })
    ...
    this.actions$.pipe(
      ...
      switchMap(({ payload }) =>
        this.usersService.createUser(payload).pipe(
          map((user) => UsersListActions.onCreateUserSuccess({ user })),
          tap(() =>
            this.snackbarService.openSuccess(USER_CREATE_SUCCESS_MESSAGE)
          ),
          catchError((error) =>
    ...
    this.actions$.pipe(
      ...
      switchMap(([{ userId, payload }, currentUser]) =>
        this.usersService.editUser(payload, userId).pipe(
          ...
          tap(() =>
            this.snackbarService.openSuccess(USER_UPDATE_SUCCESS_MESSAGE)
          ),
          catchError((error) =>
    ...
    this.actions$.pipe(
      ...
      switchMap(({ userId }) =>
        this.usersService.deleteUser(String(userId)).pipe(
          map((result) => UsersListActions.onDeleteUserSuccess({ result })),
          tap(() =>
            this.snackbarService.openSuccess(USER_ARCHIVE_SUCCESS_MESSAGE)
          ),
          catchError((error) =>
            of(UsersListActions.onDeleteUserFailure({ error })).pipe(
              tap(() =>
                this.snackbarService.openError(USER_ARCHIVE_FAIL_MESSAGE, error)
              )
            )
          )
        )
  ...
  onUndeleteUser$ = createEffect(() =>
    this.actions$.pipe(
      ofType(UsersListActions.onUndeleteUser),
      switchMap(({ userId }) =>
        this.usersService.undeleteUser(String(userId)).pipe(
          map((result) => UsersListActions.onUndeleteUserSuccess({ result })),
          tap(() =>
            this.snackbarService.openSuccess(USER_UNARCHIVE_SUCCESS_MESSAGE)
          ),
          catchError((error) =>
            of(UsersListActions.onUndeleteUserFailure({ error })).pipe(
              tap(() =>
  ...
  onResetPasswordUser$ = createEffect(() =>
    this.actions$.pipe(
      ofType(UsersListActions.onResetPasswordUser),
      switchMap(({ userId }) =>
        this.usersService.adminResetUserPassword(String(userId)).pipe(
          map((result) =>
            UsersListActions.onResetPasswordUserSuccess({ result })
          ),
          tap((response) => {
            this.dialog.open(AlertComponent, {
              ...
              data: {
                ...
                message: `Password Reset has been initiated for ${response?.result?.name}.
                    Instructions email has been sent.`,
                buttons: [{ title: 'OK', type: ButtonTypes.CONFIRM }],
              },
            });
          }),
          catchError((error) =>
            of(UsersListActions.onResetPasswordUserFailure({ error }))
          )
        )
      )
    )
  ...
  private isRequireLogout(
    ...
    return role !== loggedInUser.role || active !== loggedInUser.active;
  }
}
