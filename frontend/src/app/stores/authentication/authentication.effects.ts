...
export class AuthenticationEffects {
    ...
    () => {
      return this.actions$.pipe(
        ofType(
          ...
          AuthenticationActions.onForgotPassword
        ),
  ...
  private updateMenuNavigation(sessionUser: User): void {
    ...
    navigationItems.forEach(({ name, children, hide }) => {
      if (hide) {
        this.fuseNavigationService.updateNavigationItem(name, { hidden: true });
        return;
      }
            ...
            .map(({ name: itemName, rules, condition }) => {
              ...
              const isAccessible =
                ...
                && rules.every(({ actions, subject }) =>
                  this.permissionsService.can({ actions, subject })
                ) &&
                // condition check
                (isFunction(condition) ? condition(sessionUser) : true);

              this.fuseNavigationService.updateNavigationItem(itemName, { hidden: !isAccessible });
