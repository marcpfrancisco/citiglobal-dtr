...
import { FiltersInput } from '@bloomlocal/ng-shared';
...
@Component({
  selector: 'bloomlocal-multi-select-filters-dialog',
...
export class MultiSelectFiltersDialogComponent implements OnInit {
  static panelClass = 'bloomlocal-multi-select-filters-dialog';
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) private filterData: MultiSelectFiltersDialogData,
  ) {
    ...
    if (data?.input?.options) {
      ...
      this.inputData = data.input.options.map(option => {
        return {
          label: option.label,
          value: option.value,
          isChecked: value?.some(aValue => option.value === aValue) ?? false,
        }
      })
    }
  }
