...
@Directive({
  selector: '[bloomlocalObserveVisibility]',
})
export class ObserveVisibilityDirective
  ...
  ngOnInit(): void {
    ...
    this.intersectSubscription = this.intersectSubject$
      .subscribe(
        async (data: IntersectionData): Promise<void> => this.onIntersect(data)
      );
  }
  ...
  protected async onIntersect({
    ...
    const isVisible = await new Promise((resolve) => {
      let subObserver = new IntersectionObserver(([subEntry]) => {
        resolve(subEntry.intersectionRatio > 0 || subEntry.isIntersecting);
